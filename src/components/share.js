...
export default function Share(props) {
  const twitterUri = encodeURI('https://twitter.com/intent/tweet?text=' + JSONData.shareMessage + '&url=' + JSONData.siteUrl + '&button_hashtag=' + JSONData.primaryHashtag + '&via=' + 'ppefornhs');
  const facebookUri = encodeURI('https://www.facebook.com/sharer/sharer.php?u=' + JSONData.siteUrl);
  const linkedinUri = encodeURI('https://www.linkedin.com/shareArticle?mini=true&url=' + JSONData.siteUrl + '&title=' + JSONData.siteTitle + '&summary=' + JSONData.shareMessage + '&source=' + JSONData.primaryHashtag);

  return (
    <div className={props.share_class}>
      <span className="c-share__title">{props.title}</span>
